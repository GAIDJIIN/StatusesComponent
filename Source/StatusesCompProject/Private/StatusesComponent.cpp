...
void UStatusesComponent::RemoveStatus(FGameplayTag TagToRemove)
{
    if(!Statuses.RemoveTag(TagToRemove) || !TagToRemove.IsValid()) return;
    ClearTemporaryTagTimer(TagToRemove);
    OnRemoveStatus.Broadcast(TagToRemove);
}

void UStatusesComponent::RemoveStatuses(FGameplayTagContainer TagsToRemove)
{
    if(!GetIsContainTags(TagsToRemove,true,true) || !TagsToRemove.IsValid()) return;
