...
struct FApplyLogicInfoArray
{
	GENERATED_USTRUCT_BODY()
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Statuses Component")
		TArray<TObjectPtr<UApplyTagLogicObject>> ApplyLogicInfoArray;

	FApplyLogicInfoArray()
	{
	}

	FApplyLogicInfoArray(const TArray<TObjectPtr<UApplyTagLogicObject>> NewApplyLogicInfoArray) :
...
struct FApplicableStatusInfoByState
{
	...
	FApplicableStatusInfoByState(const FGameplayTagContainer NewApplicableStatuses,
	                             const FApplicableStatusesInfo NewStatusApplyInfo) :
